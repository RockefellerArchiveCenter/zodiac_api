...
def parse_update(data):
    ...
    """
    filtered_data = {key: value for key, value in data.items() if (value and key != 'identifier')}
    update_expression = 'SET {}'.format(','.join(f'#{k}=:{k}' for k in filtered_data))
    expression_attribute_values = {f':{k}': v for k, v in filtered_data.items()}
