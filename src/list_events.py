...
def list_events(outcome=None):
    try:
        if outcome:
            return list(scan(dynamo, FilterExpression=boto3.dynamodb.conditions.Attr('outcome').eq(outcome)))
        else:
...
def lambda_handler(event, context):
    ...
    outcome = processed_path.split('/')[-1].replace('-', ' ') if len(processed_path.split('/')) > 2 else None
