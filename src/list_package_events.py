...
from .helpers import scan, format_response
...
def list_package_events(package_id=None):
    try:
        if package_id:
            return list(scan(dynamo, FilterExpression=boto3.dynamodb.conditions.Attr('package_identifier').eq(package_id)))
        else:
...
def lambda_handler(event, context):
    ...
    package_id = processed_path.split('/')[-2] if len(processed_path.split('/')) > 3 else None
