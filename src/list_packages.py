...
def list_packages(status=None):
    try:
        if status:
            return list(scan(dynamo, FilterExpression=boto3.dynamodb.conditions.Attr('status').contains(status)))
        else:
...
def lambda_handler(event, context):
    ...
    status = processed_path.split('/')[-1].replace('-', ' ') if len(processed_path.split('/')) > 2 else None
